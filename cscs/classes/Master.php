...
Class Master extends DBConnection {
	...
	function save_slideshow() {
		...
		$check = $this->conn->query("SELECT * FROM `slideshow_list` WHERE `name` = '{$name}' " . (!empty($id) ? " AND id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err()) return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "Slideshow Name already exists.";
			return json_encode($resp);
			exit;
		}
	
		if (empty($id)) {
			$sql = "INSERT INTO `slideshow_list` SET {$data} ";
		} else {
			$sql = "UPDATE `slideshow_list` SET {$data} WHERE id = '{$id}' ";
		}
	
		$save = $this->conn->query($sql);
		if ($save) {
			$resp['status'] = 'success';
			$resp['msg'] = empty($id) ? "New Slideshow successfully saved." : "Slideshow successfully updated.";
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
		if ($resp['status'] == 'success') $this->settings->set_flashdata('success', $resp['msg']);
		return json_encode($resp);
	}
	...
	function delete_slideshow() {
		...
		$del = $this->conn->query("DELETE FROM `slideshow_list` WHERE id = '{$id}'");
		
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Slideshow successfully deleted.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	...
	// slideshow
	case 'save_slideshow':
		echo $Master->save_slideshow();
	break;
	case 'delete_slideshow':
		echo $Master->delete_slideshow();
	break;
	// end slideshow
	case 'save_product':
